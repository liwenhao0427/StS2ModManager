...
public partial class LocalizationService : ObservableObject
{
    ...
    private readonly Dictionary<string, Dictionary<string, string>> _texts = new(StringComparer.OrdinalIgnoreCase)
    {
        [LanguageZhCn] = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["Mods.SyncGithub"] = "从GitHub更新Mod",
            ["Mods.SyncGithubSingle"] = "更新当前Mod",
            ["Mods.RenameByFolder"] = "别名设为目录名",
            ["Mods.BatchRenameByFolder"] = "按目录名批量设别名",
            ["Mods.RestoreAllGithub"] = "全部恢复更新",
            ["Mods.OpenGithubLogs"] = "查看更新日志",
            ["ModDetail.Title"] = "Mod详情（统一编辑）",
            ["ModDetail.ExtractJsonFromDll"] = "从Dll提取Json覆盖",
            ["Field.Name"] = "原始名称:",
            ["Field.Alias"] = "别名:",
            ["Field.AliasHint"] = "别名单独保存在本地 settings.json，不会修改 Mod 自带 json 的 name。",
            ["Field.Tag"] = "标签:",
            ...
            ["Save.Slot"] = "栏位:",
            ["Save.Copy"] = "复制存档",
            ["Save.QuickOpenSaves"] = "打开存档目录",
            ["Save.QuickCopy"] = "复制存档（双向）",
            ["Save.QuickCopyNormalToMod"] = "复制存档（原版 -> Mod）",
            ["Save.QuickCopyModToNormal"] = "复制存档（Mod -> 原版）",
            ...
            ["Status.ModNameUpdatedByFolder"] = "已将当前Mod别名设为目录名称",
            ["Status.ModNameBatchUpdatedByFolder"] = "已批量按目录名设置 {0} 个Mod别名",
            ...
            ["Status.GithubRestoreDone"] = "已恢复 {0} 条同步记录",
            ["Status.GithubRestoreSkipped"] = "没有可恢复的同步记录",
            ...
            ["Msg.InvalidGameDirectoryConfirm"] = "选择的目录下未找到 SlayTheSpire2.exe。\n\n仍然要保留这个路径吗？保留后不会自动扫描 Mod，直到你改成正确目录。",
            ...
            ["Msg.NoModForDirectoryRename"] = "当前没有可按目录名批量设别名的Mod。",
            ["Msg.DirectoryRenameSourceRequired"] = "请至少勾选一个要处理的目录。",
            ["Msg.DirectoryRenameDone"] = "已按目录名称设置 {0} 个Mod别名。",
            ["Msg.GithubSourceRequired"] = "请至少勾选一个要同步的目录。",
            ["Msg.GithubRestoreDone"] = "已恢复 {0} 条同步记录。\n可以重新执行同步。",
            ["Msg.GithubSingleNoRepo"] = "当前Mod未配置可用的GitHub仓库链接。",
            ["Msg.GithubSingleSyncSummary"] = "当前Mod更新完成\n有更新: {0}\n地址无效: {1}\n已是最新: {2}\n日志: {3}",
            ["Msg.GithubSyncFailureReasons"] = "失败原因（去重）:",
            ["Msg.GhNotInstalled"] = "未检测到 GitHub CLI (gh)。\n\n请先安装 gh：\nhttps://cli.github.com\n\n安装后在命令行执行 gh auth login 完成登录，再重试同步。",
            ["Msg.GhNotLoggedIn"] = "GitHub CLI (gh) 未登录。\n\n请在命令行执行：\n  gh auth login\n\n按提示完成登录后，再重试同步。",
            ["Save.EnableFullConsole"] = "开启控制台",
            ["Common.Donate"] = "☕ 捐赠",
            ["Common.ToggleDark"] = "切换深色/亮色模式",
            ["Common.DarkMode"] = "🌙 深色",
            ["Common.LightMode"] = "☀ 亮色",
            ["Msg.FullConsoleAlreadyEnabled"] = "full_console 已经是 true，无需重复设置。",
            ["Msg.FullConsoleEnabled"] = "已成功写入 full_console: true\n文件: {0}\n\n重启游戏后生效。",
            ["Msg.SettingsJsonNotFound"] = "未找到 settings.save 文件：\n{0}\n\n请先运行一次游戏以生成该文件。",
            ["Status.FullConsoleEnabled"] = "已开启控制台功能",
            ["Dialog.GithubSync.Title"] = "GitHub同步进度",
            ["Dialog.GithubSync.Header"] = "正在同步 GitHub Mod",
            ["Dialog.GithubSync.SingleTitle"] = "单个Mod更新进度",
            ["Dialog.GithubSync.SingleHeader"] = "正在更新当前 Mod",
            ...
            ["Dialog.RenameByFolder.Title"] = "选择要按目录名设别名的目录",
            ["Dialog.RenameByFolder.Header"] = "请选择要处理的目录",
            ["Dialog.RenameByFolder.Hint"] = "会把所选目录下的 Mod 别名设为各自目录名称，不会修改原始 name。",
        ...
        [LanguageEnUs] = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["Mods.SyncGithub"] = "Update Mods from GitHub",
            ["Mods.SyncGithubSingle"] = "Update This Mod",
            ["Mods.RenameByFolder"] = "Alias As Folder",
            ["Mods.BatchRenameByFolder"] = "Batch Alias As Folder",
            ["Mods.RestoreAllGithub"] = "Restore All Updates",
            ["Mods.OpenGithubLogs"] = "View Update Logs",
            ["ModDetail.Title"] = "Mod Details (Unified Edit)",
            ["ModDetail.ExtractJsonFromDll"] = "Extract Json From Dll",
            ["Field.Name"] = "Original Name:",
            ["Field.Alias"] = "Alias:",
            ["Field.AliasHint"] = "The alias is stored only in local settings.json and does not modify the mod's original json name.",
            ["Field.Tag"] = "Tag:",
            ...
            ["Save.QuickOpenSaves"] = "Open Saves Folder",
            ["Save.QuickCopy"] = "Copy Saves (Both Ways)",
            ["Save.QuickCopyNormalToMod"] = "Copy Saves (Normal -> Modded)",
            ["Save.QuickCopyModToNormal"] = "Copy Saves (Modded -> Normal)",
            ...
            ["Status.ModNameUpdatedByFolder"] = "Set current mod alias from folder name",
            ["Status.ModNameBatchUpdatedByFolder"] = "Set aliases for {0} mods from folder names",
            ...
            ["Status.GithubRestoreDone"] = "Restored {0} sync records",
            ["Status.GithubRestoreSkipped"] = "No sync record to restore",
            ...
            ["Msg.InvalidGameDirectoryConfirm"] = "SlayTheSpire2.exe was not found in the selected folder.\n\nDo you still want to keep this path? Mods will not be scanned until you change it to a valid directory.",
            ...
            ["Msg.NoModForDirectoryRename"] = "There are no mods available for folder-based alias updates.",
            ["Msg.DirectoryRenameSourceRequired"] = "Please select at least one directory to process.",
            ["Msg.DirectoryRenameDone"] = "Set aliases for {0} mods from their folder names.",
            ["Msg.GithubSourceRequired"] = "Please select at least one source directory.",
            ["Msg.GithubRestoreDone"] = "Restored {0} sync records.\nYou can run sync again.",
            ["Msg.GithubSingleNoRepo"] = "The current mod has no valid GitHub repository URL configured.",
            ["Msg.GithubSingleSyncSummary"] = "Current mod sync finished\nUpdated: {0}\nInvalid links: {1}\nAlready latest: {2}\nLog: {3}",
            ["Msg.GithubSyncFailureReasons"] = "Failure reasons (deduplicated):",
            ["Msg.GhNotInstalled"] = "GitHub CLI (gh) is not installed.\n\nPlease install gh first:\nhttps://cli.github.com\n\nAfter installation, run 'gh auth login' in a terminal to sign in, then retry.",
            ["Msg.GhNotLoggedIn"] = "GitHub CLI (gh) is not logged in.\n\nPlease run in a terminal:\n  gh auth login\n\nFollow the prompts to sign in, then retry.",
            ["Save.EnableFullConsole"] = "Enable Console",
            ["Common.Donate"] = "☕ Donate",
            ["Common.ToggleDark"] = "Toggle dark/light mode",
            ["Common.DarkMode"] = "Dark",
            ["Common.LightMode"] = "Light",
            ["Msg.FullConsoleAlreadyEnabled"] = "full_console is already true, no change needed.",
            ["Msg.FullConsoleEnabled"] = "Successfully wrote full_console: true\nFile: {0}\n\nRestart the game for it to take effect.",
            ["Msg.SettingsJsonNotFound"] = "settings.save not found:\n{0}\n\nPlease run the game once to generate this file.",
            ["Status.FullConsoleEnabled"] = "Console feature enabled",
            ["Dialog.GithubSync.Title"] = "GitHub Sync Progress",
            ["Dialog.GithubSync.Header"] = "Syncing GitHub Mods",
            ["Dialog.GithubSync.SingleTitle"] = "Single Mod Update Progress",
            ["Dialog.GithubSync.SingleHeader"] = "Updating Current Mod",
            ...
            ["Dialog.RenameByFolder.Title"] = "Select Directories to Alias by Folder",
            ["Dialog.RenameByFolder.Header"] = "Choose directories to process",
            ["Dialog.RenameByFolder.Hint"] = "Aliases under selected directories will be set to each mod's folder name without changing the original json name.",
